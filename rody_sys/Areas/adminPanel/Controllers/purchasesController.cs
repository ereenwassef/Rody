...
//================================================
        ...
        public void add_purchase_operation(string date, int supplierId, int clientId, int shareehaId, float quantity, float price, float discount)
        {
            ...
            using (var purchaseTransaction = DB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (sellerType == (int)sellerTypeEnum.client)
                    {
                        ...
                        add_purchase_operation(date, supplierId ?? 0, clientId ?? 0, shareehaId, quantity, price, discount);
